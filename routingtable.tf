...
resource "aws_route_table" "user_dmz_rt" {
  ...
  # }  
  tags = {
    Name = "${local.names[0]}_pri_rt_${local.userdev_rt_name[7]}_${local.az_ac[count.index]}"
  }
}
...
resource "aws_route_table" "dev_dmz_rt" {
  ...
  # }  
  tags = {
    Name = "${local.names[1]}_pri_rt_${local.userdev_rt_name[7]}_${local.az_ac[count.index]}"
  }
}
