...
data "aws_vpc_endpoint" "user_nwf_endpoints" {
  ...
  subnet_ids = aws_subnet.user_dmz_pub[count.index + 2].id
  # filter {
  #   name   = "cidr_blocks"
  #   values = [aws_subnet.subnet_user_dmz_pub[count.index + 2].cidr_block]
  # }
}  
...
data "aws_vpc_endpoint" "dev_nwf_endpoints" {
  ...
  subnet_ids = aws_subnet.dev_dmz_pub[count.index + 2].id
  # filter {
  #   name   = "cidr_blocks"
