...
resource "aws_lb" "user_dmz_proxy_lb" {
  ...
  internal           = false
  subnets            = [aws_subnet.subnet_user_dmz_pub[4].id, aws_subnet.subnet_user_dmz_pub[5].id]
  security_groups    = [aws_security_group.dmz_lb_sg[0].id]
}
...
resource "aws_lb_listener" "dev_nexus_lb_listener" {
  ...
  port              = local.dmz_ports[count.index + 2]
  protocol          = "TCP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.dev_dmz_nexus_tg[count.index].arn
